...
///
/// Capacitor calls `shouldOverrideLoad` on every registered plugin before each
/// navigation; returning `true` cancels Capacitor's default load.
///
/// Install: after `npm run add:ios`, copy this file into
/// `ios/App/App/` and add it to the App target in Xcode. Capacitor
/// auto-discovers `CAPPlugin` subclasses at runtime.
